...
import {
    ...
    Flying,
    Aquatic
} from './game_model/cards/mechanics/skills';
...
export class TipService {
    ...
    private playResourceTrigger(game: Game, playerNo: number) {
        ...
        const playable = game
            .getPlayer(playerNo)
            .getHand()
            .filter(card => card.isPlayable(game));
    ...
    private drawCardTrigger(game: Game, playerNo: number, discarded: boolean) {
        const cardCount = game.getPlayer(playerNo).getHand().length;
    ...
    public cannotBlockTargetTip(blocker: Unit, attacker: Unit, game: Game) {
        if (!attacker.isAttacking()) {
            ...
            !attacker.hasMechanicWithId(Aquatic.getId())
        ) {
            this.announce(`Aquatic units may only block other aquatic units.`);
        } else {
            this.announce(
                `${blocker.getName()} cannot block ${attacker.getName()} due to a special effect.`
            );
        }
    }
    ...
    public cannotModifyEnchantment(
        player: Player,
        game: Game,
        enchantment: Enchantment
    ) {
        const verb =
            enchantment.getOwner() === player.getPlayerNumber()
                ? 'empower'
                : 'diminish';
        if (player !== game.getCurrentPlayer()) {
            ...
            this.announce(`You can not afford to ${verb} that enchantment.
                It would require ${enchantment
                    .getModifyCost()
                    .getNumeric()} energy while you only have ${player
                .getPool()
                .getNumeric()}.`);
        }
    }
    ...
    public cannotPlayTip(playerNo: number, game: Game, card: Card) {
        ...
        if (game.getCurrentPlayer().getPlayerNumber() !== playerNo) {
            ...
            const diff = card.getCost().difference(player.getPool());
            this.announce(
                `You need ${diff
                    .map(change => change.diff + ' more ' + change.name)
                    .join(' and ')} to play ${card
                    .getName()
                    .replace(/\./g, '')}.`
            );
        } else if (
            ...
            card.getCardType() === CardType.Enchantment &&
            !game.getBoard().canPlayPermanent(card as Enchantment)
        ) {
            this.announce(`Your board is too full to play an enchantment.`);
        } else if (
            card.getCardType() === CardType.Item &&
            !(card as Item).getHostTargeter().isTargetable(card, game)
        ) {
